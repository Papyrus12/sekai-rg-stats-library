...
exports.inverse = exports.calculate = void 0;
 ...
 * kaiden average.
 */
function calculate(yourEx, kaidenAverage, worldRecord, max, powCoef) {
    let powCoefficient = powCoef ?? 1.175;
    if (powCoefficient === -1) {
        powCoefficient = 1.175;
    }
    AssertProvidedEXScores(kaidenAverage, worldRecord, max, powCoefficient);
    throw_if_1.ThrowIf.not(Number.isSafeInteger(yourEx), "Provided EX was not an integer.", { yourEx });
    (0, throw_if_1.ThrowIf)(yourEx > max, "Provided EX was greater than MAX.", { yourEx, max });
    throw_if_1.ThrowIf.negative(yourEx, "Provided EX was negative.", { yourEx });
    (0, throw_if_1.ThrowIf)(worldRecord === kaidenAverage, "BPI is undefined for the case where Kaiden Average equals the World Record", { kaidenAverage, worldRecord });
    ...
    const logExPrime = (0, math_1.LogToBase)(yourScorePrime, wrScorePrime);
    if (isWorseThanKavg) {
 ...
 */
function PikaGreatFunction(score, max) {
 ...
 */
function inverse(bpi, kaidenAverage, worldRecord, max, powCoef) {
    let powCoefficient = powCoef ?? 1.175;
    if (powCoefficient === -1) {
        powCoefficient = 1.175;
    }
    AssertProvidedEXScores(kaidenAverage, worldRecord, max, powCoefficient);
    (0, throw_if_1.ThrowIf)(bpi < -15, "BPI must be greater than or equal to -15.", { bpi });
 ...
 */
function InversePikaGreatFunction(pgf, max) {
    return (pgf * max - 0.5 * max) / pgf;
}
/**
 * Apply assertions and constraints into values that BPI recieves.
 */
function AssertProvidedEXScores(kaidenAverage, worldRecord, max, powCoef) {
    // Is integer?
    throw_if_1.ThrowIf.not(Number.isSafeInteger(kaidenAverage), "Kaiden Average was not an integer.", {
        kaidenAverage,
    });
    throw_if_1.ThrowIf.not(Number.isSafeInteger(worldRecord), "World Record was not an integer.", {
        worldRecord,
    });
    throw_if_1.ThrowIf.not(Number.isSafeInteger(max), "MAX was not an integer.", { max });
    // Is bounded correctly?
    (0, throw_if_1.ThrowIf)(kaidenAverage > max, "Kaiden Average was greater than MAX.", {
        kaidenAverage,
        max,
    });
    (0, throw_if_1.ThrowIf)(worldRecord > max, "World Record was greater than MAX.", {
        worldRecord,
        max,
    });
    (0, throw_if_1.ThrowIf)(kaidenAverage > worldRecord, "Kaiden Average was greater than WR.", {
        kaidenAverage,
        worldRecord,
    });
    throw_if_1.ThrowIf.negativeOrZero(kaidenAverage, "Kaiden Average was negative or zero.", {
        kaidenAverage,
    });
    throw_if_1.ThrowIf.negativeOrZero(max, "MAX was negative or zero.", { max });
    throw_if_1.ThrowIf.negativeOrZero(powCoef, "Power Coefficient was not positive.", { powCoef });
...
//# sourceMappingURL=poyashi-bpi.js.map