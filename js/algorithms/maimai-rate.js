...
function calculateRatingCurve(iclInt) {
    ...
    return lowerCurve.map((rateValue, index) => {
        ...
        return (rateValue + ((upperRateValue - rateValue) * (iclInt - lowerIcl)) / (upperIcl - lowerIcl));
    });
}
/**
 ...
 */
function calculate(score, maxScore, internalChartLevel) {
    ...
    (0, throw_if_1.ThrowIf)(maxScore > 104, "Max score cannot be greater than 104%.", { maxScore });
    (0, throw_if_1.ThrowIf)(score > maxScore, "Score cannot be greater than max score.", { score, maxScore });
